...
import numpy as np
from tensorhue.converters import _tensor_to_numpy_jax
...
def test_jax_device_array():
    ...
    assert np.array_equal(_tensor_to_numpy_jax(device_array), np.array(data))
...
def test_tensor_dtypes():
    ...
    for dtype_jnp, dtype_np in dtypes.items():
        ...
        converted = _tensor_to_numpy_jax(jnp_casted)
...
def test_jax_incompatible_arrays():
    ...
    with pytest.raises(NotImplementedError) as exc_info:
        _tensor_to_numpy_jax(shaped_array)
...
def test_runtime_error_for_non_convertible_tensor():
    ...
    with pytest.raises(NotImplementedError) as exc_info:
        _tensor_to_numpy_jax(non_convertible)
